...
- 提交交互区：未选中置灰，选中后可提交；提交链路先意愿弹窗，再提交成功弹窗。
...
- 登录回流后必须重新以 `uid + match_id` 获取正式分组。
